...
export const HomePage = () => {
  ...
  React.useEffect(() => {
    const getAllData = async (): Promise<void> => {
      ...
      const choosedInfo = await requests.getMovieInfo(
        choosedMovie.id,
        FeatureType.tv
      )
  ...
  return (
    <React.Fragment>
      ...
      {featureData ? <FeaturedMovie movie={featureData} /> : <Loading />}
      <Main>
        {movieList?.map((item: IMovieList, index: React.Key) => {
          return <CategoryRow key={index} title={item.title} item={item} />
        })}
      </Main>
      <Footer />
    </React.Fragment>
